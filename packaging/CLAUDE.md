...
Mac 的 dist 里只有 dmg，永远没有 exe。「打开工作台.bat」也不生成 exe。要 exe：GitHub Release 标签 windows 下 zip，或在 Windows 上双击「生成Windows程序.bat」，等到出现 OK，去桌面看 SportDesk 文件夹。批处理必须是 CRLF，不能含中文，否则会报「不是内部或外部命令」。
